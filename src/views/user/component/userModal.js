...
handleStateClear=()=>{
  ...
  //   this.setState({userName:'',email:'',password:'',role:null});
  //   }
}
...
handleSubmit=()=>{
  ...
  validate = (name, value) => {
    ...
    switch (name) {
      case 'userName':
        ...
        case 'password':
        const PASSWORD_REGEX = RegExp("^[a-zA-Z'.\\s]{1,40}$");
        if (value.length < 5) {
          this.setState(
            { error: { password: `username is too short (Minimum ${5} characters needed.)` } }
          )
        }
        else if (value.length > 15) {
          this.setState(
            { error: { password: `username is too long (Maximum ${15} characters allowed.)` } }
          )
        } else if (!PASSWORD_REGEX.test(value)) {
          this.setState(
            { error: { password: `Only character allowed.` } }
          )
        }
        else {
          this.setState(
  ...
  checkEmptySpace = ()=> {
    if (this.state.leaveType.trim() === "") {
      this.setState(
        { error: { LeaveType: `Please enter Leavetype.` } }
      )
    }
  }
  render() {
